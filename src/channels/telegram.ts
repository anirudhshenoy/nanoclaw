...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:text', async (ctx) => {
      // Skip built-in bot commands handled by grammy's command() handlers above
      if (/^\/(?:chatid|ping)(?:@\S+)?(?:\s|$)/i.test(ctx.message.text)) return;
    ...
    this.bot.on('message:voice', async (ctx) => {
      ...
      try {
        ...
        if (file.file_path) {
          const mediaDir = path.join(GROUPS_DIR, group.folder, 'media');
          fs.mkdirSync(mediaDir, { recursive: true });
          const savePath = path.join(mediaDir, `${Date.now()}.ogg`);
          const downloadUrl = `https://api.telegram.org/file/bot${this.botToken}/${file.file_path}`;
          await new Promise<void>((resolve, reject) => {
            const out = fs.createWriteStream(savePath);
            https
          ...
          fs.unlinkSync(savePath); // clean up after transcription
          content = `[Voice: ${transcript}]`;
          logger.info(
            { group: group.name, chars: transcript.length },
...
export async function sendPoolMessage(
  ...
  if (poolApis.length === 0) {
    // No pool bots — fall back to plain send (caller handles routing)
    return;
  }
