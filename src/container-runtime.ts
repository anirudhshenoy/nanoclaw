...
function detectHostGateway(): string {
  ...
  // Detect the bridge IP from the host's network interfaces (bridge100 on macOS).
...
function detectProxyBindHost(): string {
  // Apple Container VMs use a separate network bridge (e.g. 192.168.64.1), not loopback.
  // host.docker.internal resolves to that bridge IP, so 127.0.0.1 is unreachable from the VM.
